...
def _retag_to_public_docker(_split_list, prefix='rocm/pytorch', delim='pytorch'):
    _result = []
    for item in _split_list:
        ...
        _ver = _get_version(tail_tag)
        _temp.append(_ver)
        _result.append(''.join(_temp))
    return _result 

# Check before running it
def _final_fix(_list):
    _result = []
    for item in _list:
        if 'centos' in item:
            _result.append(re.sub(r'centos7.[0-9]+.[0-9]+', 
                                  r'centos7', item))
        elif '1.8' in item:
...
if __name__ == '__main__':
    ...
    FINAL_RESULT = _retag_to_public_docker(SPLIT_STR_LIST)
